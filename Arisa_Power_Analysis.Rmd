...
Our initial analysis shows that for 1000 iterations of 50 subjects, the p-values peak closer to 0. This indicates that the probability for a false negative is low. 
...
```{r p value histogram, include = TRUE}
ggplot()+
  aes(p_values) +
  geom_histogram() +
  labs(
...
We further explored how varying split between control and treatment affected power when total number of subjects is held constant at 20. According to our figure below, power continues to increase with % in treatment until 50%, at which it starts to drop off. Thus, we will aim for 50/50 split in responses from treatment and control in our experiment. 

```{r split analysis, include = TRUE}
set.seed(123)
